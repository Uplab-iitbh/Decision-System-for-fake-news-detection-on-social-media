...
def calculate_novelty_scores(data, top_ids_and_scores):
    # Implement novelty score calculation based on user preferences or system history
    # This could involve comparing retrieved passages with previously seen content
    # Return a list of novelty scores corresponding to each retrieved passage
    novelty_scores = []
    for i, scores in enumerate(top_ids_and_scores):
        threshold = 0.7
        novelty_scores.append([0.0 if score >= threshold else 1.0 for score in scores])  # Higher score for less novel passages
        
        # Printing of negative comments for retriver passages
        for j, score in enumerate(scores):
            if score < threshold:
                print(f"Passage {j} in question {i} is not very novel.")
